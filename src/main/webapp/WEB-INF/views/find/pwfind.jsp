...
<script>
    function pverifyAndSubmit(event) {
        ...
        var puserid = document.querySelector('input[name="puserid"]').value;
        var email = document.getElementById("email").value;

        if (!name || !email || !puserid) {
            alert("이름과 이메일과 아이디를 입력하세요.");
            return;
        }
        ...
        $.ajax({
            ...
            data: { name: name, email: email, puserid: puserid },
	 ...
	 let pisVerified = false;	 // 이메일 인증을 하지않으면 완료를 누를수 없게 만듬 
...
<script>
    function cverifyAndSubmit(event) {
        event.cpreventDefault();  // 폼의 기본 제출 동작을 막음
     ...
     function csendVerificationCode() {
         ...
         $.ajax({
			 ...
			 let cisVerified = false; // 이메일 인증을 하지않으면 완료를 누를수 없게 만듬 
...
<!--		일반회원 비밀번호 찾기 -->
        <div class="container">
            <div class="header-container">
                <div class="btn-group">
                    <button type="button" id="generalBtn" class="form-button active" data-form="generalForm" onclick="geloform()">일반회원 비밀번호 찾기</button>
                    <button type="button" id="businessBtn" class="form-button" data-form="businessForm" onclick="buloform()">기업회원 비밀번호 찾기 </button>
                </div>
            </div>
...
<!--			일반 회원-->
            ...
            <form method="post" id="generalForm" class="visible" action="${pageContext.request.contextPath}/ppwchange">
                일반회원 이름<input type="text" name="pname" placeholder="이름을 입력해주세요" required><br>
                일반회원 아이디<input type="text" name="puserid" placeholder="아이디를 입력해주세요" required><br>

                <!-- 이메일 인증 -->    
                <div class="form-group">
                    일반회원 이메일<input type="text" id="email" name="email" placeholder="Email을 입력해주세요" style="width: 73%;">
                    <div class="validation-message"></div>
                    <span>
                        <button type="button" id="psendVerificationButton" style="width: 50%; border: 1px solid #ccc;"
                            onclick="psendVerificationCode()" class="submit-btn">인증번호 전송</button>
                    </span>
                </div>
                <div class="form-group">
                    ...
                    <div class="validation-message"></div>
                    <span>
                        <button type="button" style="width: 15%; border: 1px solid #ccc;"
                            onclick="pverifyCode()" class="submit-btn">확인</button>
                    </span>
			...
			<!--			기업 회원-->
            <form method="post" id="businessForm"  action="${pageContext.request.contextPath}/cpwchange">
                기업회원 대표자 명<input type="text" name="cusnm" placeholder="대표자명을 입력해주세요" required><br>
                기업회원 아이디<input type="text" name="cuserid" placeholder="아이디를 입력해주세요" required><br>
                기업회원 사업자 번호<input type="text" name="bsnum" placeholder="사업자번호를 입력해주세요" required><br>
				
				<!-- 이메일 인증 -->
	               <div class="form-group">
	                   일반회원 이메일<input type="text" id="cmail" name="cmail" placeholder="Email을 입력해주세요" style="width: 73%;">
	                   <div class="validation-message"></div>
	                   <span>
	                       <button type="button" id="csendVerificationButton" style="width: 50%; border: 1px solid #ccc;"
	                           onclick="csendVerificationCode()" class="submit-btn">인증번호 전송</button>
	                   </span>
	               </div>
	               <div class="form-group">
	                   <input type="text" id="corp_checkNo" name="corp_checkNo" placeholder="인증번호를 입력하세요" style="width: 73%;">
	                   <div class="validation-message"></div>
	                   <span>
	                       <button type="button" style="width: 15%; border: 1px solid #ccc;"
	                           onclick="cverifyCode()" class="submit-btn">확인</button>
	                   </span>
	                   <button type="submit" class="submit-btn">완료</button>
					</div>
            </form>
